...
install() {
  ...
  rm -rf "${INSTALL_DIR}/CPU-RAM_Monitor@Rishu"
  cp -rf "${SRC_DIR}/src" "${INSTALL_DIR}/CPU-RAM_Monitor@Rishu" &>> "$LOG_FILE"
  glib-compile-schemas --strict --targetdir="${INSTALL_DIR}/CPU-RAM_Monitor@Rishu/schemas" "${INSTALL_DIR}/CPU-RAM_Monitor@Rishu/schemas"
  mkdir -p "$HOME/.local/var/log/"
  touch "$HOME/.local/var/log/CPU-RAM_Monitor.log"
  is_failed "Done" "Skipping: Can not install to ${INSTALL_DIR}. See log for more info."
}

# build for release
build() {
  print "Creating CPU-RAM_Monitor@Rishu.zip"
  mkdir -p "${SRC_DIR}/out"
  cd "src"
  zip -6rX "$SRC_DIR/out/CPU-RAM_Monitor@Rishu.zip" * &>> "$LOG_FILE"
  cd ..
